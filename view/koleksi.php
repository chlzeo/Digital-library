            ...
            <?php foreach($favorit as $b): ?>
                <div class="flex flex-col items-center">
                    ...
                    <div class="flex gap-2 w-36">
                        <a href="detail_buku.php?id_buku=<?= htmlspecialchars($b['BukuID']) ?>" class="flex-1 bg-gradient-to-r from-blue-700 to-blue-400 text-white px-2 py-1 rounded-lg hover:from-blue-800 hover:to-blue-500 text-center font-semibold transition shadow text-xs flex items-center justify-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4"/></svg>
                            Pinjam
                        </a>
                        <a href="hapus_koleksi.php?id=<?= htmlspecialchars($b['KoleksiID']) ?>" 
                            onclick="return confirm('Yakin ingin menghapus?');" 
                            class="flex-1 bg-gradient-to-r from-red-100 to-red-200 text-red-700 px-2 py-1 rounded-lg hover:from-red-200 hover:to-red-300 text-center font-semibold transition shadow border border-red-200 flex items-center justify-center gap-1 text-xs">
